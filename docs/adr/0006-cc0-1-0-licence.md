...
Superseded by [ADR 0008](./0008-mit-replaces-the-cc0-dedication.md), which relicenses the
repository under MIT. The dedication described here still governs every copy taken while it stood.
...
- **CC0 cannot reach the brand marks.** Almost everything in [`assets/images/png/`](../../assets/images/png) is a third-party logo (LinkedIn, GitHub, X, Reddit, Medium, Strava, Unsplash, Bluesky, Stack Overflow, 500px), and no dedication granted here can place someone else's trademark in the public domain. Those files remain governed by their owners' brand guidelines whatever [`LICENSE`](../../LICENSE) says. In practice a reuser swaps them for the icons of their own links; what the dedication actually covers is [`phone.png`](../../assets/images/png/phone.png), [`web.png`](../../assets/images/png/web.png) and the markup.
- Reusers copying [`index.html`](../../index.html) wholesale inherit personal contact details. That follows from publishing a signature at all rather than from the licence, but the licence removes any friction that might have made someone stop and edit first.
