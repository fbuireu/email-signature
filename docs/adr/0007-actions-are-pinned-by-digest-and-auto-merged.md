...
- The Dependabot auto-merge runs on `secrets.PAT` rather than `GITHUB_TOKEN`, so the merge is the Owner's and the workflows that run on `main` fire on it. That PAT is a standing credential with write access, and its expiry is an unmonitored single point of failure: when it lapses, that automation stops silently. Renovate's own pull requests need no approval, because the `main` ruleset requires none; the auto-approve workflow that used to grant one is gone, and with it the only other use of the PAT.
