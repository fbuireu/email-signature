...
- **The preview can silently drift from the Signature, and it has.** The PNG has a single commit, the one that first brought the Signature in; every later edit to `index.html` has left it untouched. Those edits were tag-balance fixes rather than visual changes, so the drift may be nil, but nothing here can say so and a stale preview misrepresents the product on the README with no failing check anywhere. The obligation is recorded in [`CLAUDE.md`](../../CLAUDE.md) instead.
